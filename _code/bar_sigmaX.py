...
for filename in os.listdir(input_folder):
    if filename.endswith(".csv"):
        ...
        try:
            ...
            if len(df) > 10:
                valid_y = df["PositionZ"].dropna().values
                sigma_y = np.sum(np.abs(np.diff(valid_y)))
...
ax.set(xlabel='', ylabel=r'$\sum | \Delta Z |$ (m)')
plt.xticks(rotation=45)
plt.ylim(0, df_all["sigma_y"].max() * 1.1)

plt.legend(title="", fontsize=12, title_fontsize=2, ncol=5, loc="upper left")

plt.savefig(f'{output_folder}/bar_all_positionZ.png', dpi=300, bbox_inches='tight')
